...
# Data was downloaded from LIST WEBSITE. Becasue the file was too large to load on most computers, it was subset into yearly data for later processing
...
system.time({
  fires <- foreach(i=1:length(fireFiles), .combine=cbind, .packages=c("lubridate","sp","raster", "rgdal", "gdalUtils")) %dopar% {
    ...
    fireSp <- spTransform(fireSp, CRS(brazil))
