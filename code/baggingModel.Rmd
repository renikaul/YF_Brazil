...
Environmental covariates were already processed in `GiantDataFrameMaker.R`. They can now be loaded in in one go.
...
bagging<-function(form.x.y,training.pres,training.abs,new.data){
  ...
  #for writing script
    #need to permutate acroos ALL columns in dataset; not just those 110 sampled. 
    training.pres <- train.pres
    training.abs <- train.bg
    
  #balance presence/background 1:10
  ...
  #premute data  
    permute.data <- balanced.data <-         rbind(training.pres[train_pos.p,],training.abs[train_pos.b,])#make data
    permuted.data[,vari] <- sample(balanced.data[,vari],110,FALSE) #permute the col named included in vari (ie. variable.names)
...
cores.to.use <- 2 #change dep on computer
...
trainModel <- foreach(i=1:iterations) %dopar% {
    ...
    output1 <- bagging(form.x.y=glm.formula,training.pres=train.pres,training.abs=train.bg,
                       new.data=training)
...
### Baseline AUC for the model

```{r permutation}
permOneVar=function(formula = glm.formula, train.pres = train.pres, train.bg = train.bg, training = training, cores=2, no.iterations= 100){
      # glm.formula:
      # train.pres
      # train.bg
      # training
  ...
  variablesName <- c(variables, "full model")
  train.auc <- data.frame(auc = rep(NA, length(variablesName)), variableDrop = rep(NA, length(variablesName)))
  #create empty plot for AUC 
  plot(NA, ylim=c(0,1), xlim=c(0,1))
  legend("bottomright", title ="Variable Dropped", legend=variablesName, col=c(1:length(variablesName)), lty=1, lwd=2)
  
  for (i in 1:(length(variables)+1)){
    print(variables[i]) #let us know where the simulation is at. 
  #drop one variable from formula object
    newVariables <- paste(variables[-i], collapse ="+")
    new.glm.formula <- as.formula(paste0("case~", newVariables))
  ...
  trainModel <- foreach(i=1:iterations) %dopar% {
      result <- multiResultClass()
      output1 <- bagging(form.x.y=new.glm.formula,training.pres=train.pres,training.abs=train.bg,new.data=training)
      result$predictions <-output1[[1]]
      result$coefs <- output1[[2]]
      return(result)
  }
  ...
  perf2 <- performance(preds, "tpr", "fpr")
  plot(perf2, add=TRUE, col=i)
  train.auc$auc[i] <- unlist(performance(preds, "auc")@y.values)
  train.auc$variableDrop[i] <- variablesName[i]
  }
  #return training coefs and AUC for each iteration
  train.auc[order(train.auc$auc),]
  return(train.auc)
...
For the first pass at variable importance, we will use a drop one variable scheme. Starting with the all the variables, we will drop one and calculate the AUC on the training data. The population density ($Log_{10}$ transformed) seems to be the most important predictor of any YF cases in a municipality. This is expected and doesn't add any intuition to the system. 
   
```{r training variable importance, echo=FALSE, eval=FALSE}
trainingCoefs<- t(trainingCoefs)

boxplot(trainingCoefs[,-c(1)], ylim = c(-50,50))
meanCoefs <- apply(trainingCoefs,2, mean)
meanCoefs
```

```{r Drop One Variable Function}
#Function to be fixed to run permutation tests.  
dropOneVar=function(formula = glm.formula, train.pres = train.pres, train.bg = train.bg, training = training, cores=2, no.iterations= 100){
      # glm.formula:
      # train.pres
      # train.bg
      # training
...
allDrop <- dropOneVar(formula = glm.formula, train.pres= train.pres, train.bg=train.bg ,training=training, cores=2, no.iterations = 100 )

allDrop[order(allDrop$auc),]
```

```{r}
glm.formula2 <- as.formula("case~  NDVIScale+ 
                          popLog10+
                          RFsqrt+ RF+
                          tempScale+
                          fireDenScale+
...
eachGroupDrop3 <- dropOneVar(formula = glm.formula2, train.pres= train.pres, train.bg=train.bg ,training=training, cores=2, no.iterations = 100 )
