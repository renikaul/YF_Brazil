...
permOneVar=function(formula = glm.formula, bag.fnc=bagging, traindata = training,  cores=2, no.iterations= 100){
  # glm.formula:
  ...
  train.auc <- data.frame(auc = rep(NA, length(variablesName)), variablePermuted = rep(NA, length(variablesName)))
  Coefs <- list()
  perm.data <- list()
  #create empty plot for AUC 
  plot(NA, ylim=c(0,1), xlim=c(0,1))
  legend("bottomright", title ="Variable Permutated", legend=variablesName, col=c(1:length(variablesName)), lty=1, lwd=2)
  
  for (i in 1:length(variablesName)){
    print(c(i,variablesName[i])) #let us know where the simulation is at. 
    
  #if statments to permute data as needed ----
    if(i==1){
      #run full model
      permuted.data <- traindata
  ...
  #Set up parallel cores to return 2 lists: 1) predictions, 2) coef of model ----
    ...
    preds <- prediction(output.preds, traindata$case)
    ...
    train.auc$variablePermuted[i] <- variablesName[i]
    
    #list of coefs
    Coefs[[i]] <- trainingCoefs
...
PermFullModel <- permOneVar(formula = glm.formula,traindata = training.data, cores=3, no.iterations = 100 )

PermFullAUC <- PermFullModel[[1]]
PermFullAUC[order(PermFullAUC$auc),]
```

Drop variables that have higher AUC than full model when permuted. 
