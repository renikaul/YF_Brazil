...
# temp <- temp %>%
#   group_by(month.no) %>%
#   mutate(scale.temp.max=stdz(tempMax)) %>%
#   mutate(scale.temp.mean=stdz(tempMean))
...
pop <- pop %>%
  ...
  dplyr::rename(cal.month=month)

```


Case Data. There are missing months/munis of cases that need to be created
...
```{r}
all.data <- join_all(list(ndvi, pop, rf, temp, cases), by=c("muni.no", "month.no"), type="full")

#clean it up
all.data <- all.data %>%
  #drop if doesn't match envCovariates
  filter(!is.na(muni.name)) %>%
  #get only columns we need
  select(muni.no, muni.name, month.no, case, logpopBin, NDVI, log.min.rf:scale.mean.rf, tempMax, tempMean)
...
cores.to.use <- 5 #bc my macbook sucks
