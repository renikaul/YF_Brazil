...
permOneVar=function(formula = glm.formula, bag.fnc=bagging,permute.fnc=permutedata, traindata = training,  cores=2, no.iterations= 100, perm=10){
  ...
  for (i in 1:length(variablesName)){
    ...
    #Set up parallel cores to return 2 lists: 1) predictions, 2) coef of model ----
      ...
      trainModel <- foreach(j=1:iterations) %dopar% {
      ...
      perm.auc[k,i] <- unlist(performance(preds, "auc")@y.values)
...
PermFullModel1 <- permOneVar(formula = glm.formula,traindata = training.data, cores=3, no.iterations = 10, perm = 10 )

PermFullAUC1 <- PermFullModel[[2]]
PermFullAUC1[,2] <- round(as.numeric(as.character(PermFullAUC1[,2])),digits=3)
PermFullAUC1[,3] <- round(as.numeric(as.character(PermFullAUC1[,3])),digits=3)

PermFullAUC1[order(PermFullAUC1$meanAUC),]
