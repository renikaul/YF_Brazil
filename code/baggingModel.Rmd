...
```{r train model}
glm.formula <- as.formula("case~NDVI+NDVIScale+popLog10+RFScale+tempMean+tempScale+spRich+primProp") 
```
...
cores.to.use <- 4 #change dep on computer
...
glm.formula <- as.formula("case~NDVI+NDVIScale+popLog10+RFScale+tempMean+tempScale+spRich+RFsqrt+fireDens") 

droponeVar=function(glm.formula = glm.formula, train.pres = train.pres, train.bg = train.bg, training = training){
  
  variables <- trimws(unlist(strsplit(as.character(glm.formula)[3], "+", fixed = T)), which = "both")
  train.auc <- data.frame(auc = rep(NA, length(variables)), variable = rep(NA, length(variables)))
  ...
  perf2 <- performance(preds, "tpr", "fpr")
  plot(perf2, main=paste("Dropped:", variables[i]))
  train.auc$auc[i] <- unlist(performance(preds, "auc")@y.values)
  train.auc$variable[i] <- variables[i]
  }
...
```{r}
varImpDrop <- droponeVar(glm.formula = glm.formula)
```
