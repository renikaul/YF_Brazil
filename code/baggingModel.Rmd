...
Rainfall is of a similar range (0 -1.5), with an exponential distribution. How to rescale (if at all)?

```{r}
rf <- readRDS("../data_clean/environmental/allRainfall.rds")
hist(rf[,5]) #exponential with long tail
range(rf[,5])
hist(rf[,6]) #exponential
range(rf[,6])
hist(rf[,7]) #exponential
range(rf[,7])
```

NDVI data is ranged 0 - 1 and is normally distributed, probably doesn't need rescaling.
```{r}
...
#13 temperature rows with NAs (likely due to clouds)
temp[temp$tempMin==Inf,] #how to deal with this? spatial permutation (ie take mean of neighbors)
```
