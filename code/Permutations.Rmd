...
RankImp=function(x, formula){
  #variable names
  variables <- trimws(unlist(strsplit(as.character(formula)[3], "+", fixed = T)), which = "both")
  #pull out Relative Importance
  varImp <- x[[1]]
  #match relative importance and variable name
  RI <- data.frame("Variable"=variables, varImp=varImp)  
  #order and add rank
  RI <- RI %>%
...
FullModel <- readRDS("../data_out/SpaceAndTimeSplit/PermFullModel500.rds")
HFullModel <- readRDS("../data_out/TwoModelSoln/HPermFullModel.rds")
LFullModel <- readRDS("../data_out/TwoModelSoln/LPermFullModel.rds")

Full <- RankImp(FullModel, full.formula)
HFull <- RankImp(HFullModel, full.formula)
LFull <- RankImp(LFullModel, full.formula)

RankFull <- rbind(Full,LFull, HFull)
RankFull$Model <- factor(c(rep("Full", dim(Full)[1]),
                           rep("Low NHP", dim(Full)[1]),
                           rep("High NHP", dim(Full)[1])),
                         levels=c("Full", "Low NHP", "High NHP"))
#order the variable levels
RankFull <- within(RankFull,
                    Variable <- factor(Variable, 
                      levels = c("popLog10", "spRich", "tempMean","RFsqrt","RF","fireDen","primProp","NDVIScale","fireDenSqrt","tempScale","RFScale","fireNum","NDVI","fireDenScale")))


VariRankFull <-  Full %>%
              select(c(Variable, SingleModel=varRank)) %>%
              dplyr::left_join(LFull %>%
                                 select(c(Variable, LowNHP=varRank)), by="Variable") %>%
               dplyr::left_join(HFull %>%
                                 select(c(Variable, HighNHP=varRank)), by="Variable") 
...
If there is no change in the rank order the coordinate plot should be relatively flat. This is the case between the full and low and NHP models 

```{r cord plot, echo=FALSE}
...
VariRankPopless <-  STPopless %>%
              select(c(Model, SingleModel=Rank)) %>%
              dplyr::left_join(LPopless %>%
                                 select(c(Model, LowNHP=Rank)), by="Model") %>%
              dplyr::left_join(HPopless %>%
                                 select(c(Model, HighNHP=Rank)), by="Model")

library(GGally)

ggparcoord(VariRankPopless, columns = 2:4, groupColumn = 'Model', scale = 'globalminmax')+
  xlab("Rank")

popless.variables <- trimws(unlist(strsplit(as.character(popless.formula)[3], "+", fixed = T)), which = "both")
