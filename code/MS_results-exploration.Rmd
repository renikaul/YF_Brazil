...
tiff(file = "figures/trendsAcrossSpace.tiff", width = 2007, height = 1425, units = "px", res = 300, family = "sans", compression = "lzw")
...
month.peak <- pred.all %>%
  ...
  slice(1) %>%
  select(muni.no, peak.month = month.no)
...
cal.month.peak <- pred.all %>%
  ...
  slice(1) %>%
  select(muni.no, peak.cal.month = cal.month)
...
2008 was definitely the worst year (and 2001). This matches what we know about the El Nino cycles, and really isn't different than what other studies have found (or even just an analysis of cases).
