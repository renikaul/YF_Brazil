...
## Model with complete cases (dropping 2014)

The primate proportion variable DNE for 2014 (land use data not available), since we'd like to use primprop in the 2 model approach we've dropped the 2014 data (single case).   
...
PermFullModel <- permOneVar(formula = glm.formula,traindata = trainTrim, cores = 10, no.iterations = 500, perm = 10, viz = TRUE, title="Full Model")
saveRDS(PermFullModel, "../data_out/SpaceAndTimeSplit/Trim2014/Perm10FullModel500.rds")

```

```{r trim kitchen sink predictions}

PredFull <-  BaggedModel(form.x.y = glm.formula, training = trainTrim, new.data = trainTrim, no.iterations = 500)

saveRDS(PredFull, file = "../data_out/Predictions/SpaceAndTime/Trim2014/fullPredictions.rds")
...
glm.formula.popless <- as.formula("case~  NDVI+NDVIScale+ 
                          ...
                          fireDenSqrt+fireDenScale+
                          spRich+primProp") 
permPoplessModel <- permOneVar(formula = glm.formula.popless,traindata = trainTrim, cores=10, no.iterations = 500, perm = 10,viz=TRUE, title="Popless Model" ) 
saveRDS(permPoplessModel, "../data_out/SpaceAndTimeSplit/Trim2014/Perm10PoplessModel500.rds")

```

```{r trim popless predictions}

PredPopless <-  BaggedModel(form.x.y = glm.formula.popless, training = trainTrim, new.data = trainTrim, no.iterations = 500)

saveRDS(PredPopless, file = "../data_out/Predictions/SpaceAndTime/Trim2014/poplessPredictions.rds")

```



## Models with perferct sep tossed and with complete cases (dropping 2014)
...
PermFullModel <- permOneVar(formula = glm.formula,bag.fnc = baggingTryCatch,traindata = trainTrim, cores = 10, no.iterations = 500, perm = 10, viz = TRUE, title="Full Model")
saveRDS(PermFullModel, "../data_out/SpaceAndTimeSplit/Trim2014/Perm10FullModel500TryCatch.rds")

```

```{r trim tryCatch kitchen sink predictions}

PredFull <-  BaggedModel(form.x.y = glm.formula, bag.fnc = baggingTryCatch,training = trainTrim, new.data = trainTrim, no.iterations = 500)

saveRDS(PredFull, file = "../data_out/Predictions/SpaceAndTime/Trim2014/fullPredictionsTryCatch.rds")


```

# Popless 
```{r trim tryCatch drop Pop}
#drop one covar importance
glm.formula.popless <- as.formula("case~  NDVI+NDVIScale+ 
                          RFsqrt+RFScale+
                          tempMean+tempScale+
                          fireDenSqrt+fireDenScale+
                          spRich+primProp") 
permPoplessModel <- permOneVar(formula = glm.formula.popless, bag.fnc =baggingTryCatch ,traindata = trainTrim, cores=10, no.iterations = 500, perm = 10,viz=TRUE, title="Popless Model" ) 
saveRDS(permPoplessModel, "../data_out/SpaceAndTimeSplit/Trim2014/Perm10PoplessModel500TryCatch.rds")

```

```{r trim tryCatch popless predictions}

PredPopless <-  BaggedModel(form.x.y = glm.formula.popless, training = trainTrim, new.data = trainTrim, no.iterations = 500, bag.fnc = baggingTryCatch)

saveRDS(PredPopless, file = "../data_out/Predictions/SpaceAndTime/Trim2014/poplessPredictionsTryCatch.rds")
