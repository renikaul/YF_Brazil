...
var.one.model <- as.data.frame(pred.one.model) %>%
  mutate(pred.scale = prediction * (sum(case)/sum(prediction))) %>%
  group_by(muni.no) %>%
  summarise(pred.var = var(pred.scale, na.rm = T)) %>%
...
**Caption:** Model variance through space (a,b,c) and time(d,e,f). Plots of variance municipality-level predictions using the single process model (a), dual process model with low reservoir richness (b), and dual process model with high reservoir richness (c). Mean prediction across the entire study area of Brazil for the single process model (e), dual process model with low reservoir richness (e), and dual process model with high reservoir richness (f). Gray lines represent an individual year of data with overall mean in black.
...
Find constants $c$, such that $ \sum{intensities} * c = \sum{pos.munis} $

```{r, include = F, eval = F}
calculateConstant <- function(pred.df){
  sumPositive <- sum(pred.df$case)
  sumPrediction <- sum(pred.df$prediction)
  c <- sumPositive/sumPrediction
  return(c)
}

calculateConstant(pred.lowNHP)
```
...
Eventually the one.model will not have the split. I just can't figure it out yet.
