...
nhp.split.map <- st_transform(nhp.split.map, 29101)
braz.outline <- st_transform(braz.outline, 29101)
...
tiff(file = "../data_out/MS_results/figures/manuscript/PercentileMap2.tiff", width = 170, height = 110, units = "mm", res = 250, family = "sans", compression = "lzw") #to get less than 10 MB
...
ggplot() +
  ...
  north(bb2, symbol = 3 , scale = 0.1, 
        anchor = c(x = 6700000, y = 7000000))
...
top.row <- ggplot() +
    ...
    scale_fill_distiller(palette = "PuBu", direction = 1, type ="seq",
                     na.value = "gray70", 
                     guide = guide_colorbar(
                       title = "Seasonality\nin Intensity\n(Variance)",
                       direction = "vertical",
...
```{r}
#pdf(file = "../data_out/MS_results/figures/manuscript/LatticePlot.pdf", width = 6.69, height = 4.75, family = "sans")

tiff(file = "../data_out/MS_results/figures/manuscript/LatticePlot.tiff", width = 2007, height = 1425, units = "px", res = 300, family = "sans", compression = "lzw")


ggdraw() +
  draw_plot(top.row, x = 0, y = 0.5, width = 1, height = 0.5) +
  draw_plot(bottom.row, x = 0.04, y = 0, width = 0.96, height = 0.5) +
  draw_plot_label(label = letters[1:6],
                  x = c(0.12,0.42,0.70,0.12,0.42,0.70),
...
for (i in 1:max(pred.map.all$month.no)){
  ...
  png(file = paste0("../data_out/MS_results/figures/manuscript/gifs/month", 
...
CMDLINE: `convert -antialias -delay 1 *.png MapPercentile.mp4` (for mp4)

# Supp. Figure: Covariate Correlation and Collinearity

## Load Data

Get all the csvs into one giant dataframe. Need to check which municipality these all belong too.

```{r}
brazil <- readOGR(dsn="../data_clean", layer="BRAZpolygons")#this must be loaded
muni.no <- brazil$muni_no

##minT
files <- list.files("../data_raw/environmental/allEnvironmental/Temperature/min", full.names = T)
filenames <- list.files("../data_raw/environmental/allEnvironmental/Temperature/min")
merge.files <- lapply(files, read.csv, header = T, stringsAsFactors = F)
minT <- do.call(cbind, merge.files)
colnames(minT) <- gsub("month", "", gsub(".csv", "", filenames))
minT$muni.inds <- muni.no
minT <- gather(minT, year, minTemp, 1:168)

##maxT
```

Correlation Matrix Using Spearman's:
