...
pred.map.subset <- pred.map %>%
  ...
  mutate(monthLabel = factor(month.abb[cal.month], levels = c("Feb", "Jun", "Dec")))
```
...
ggplot() +
  ...
  geom_sf(data = nhp.split.map, fill = NA, color = "black", size=0.25) +
...
Caption: Probability of yf spillover averaged over calendar month for December (a,d), February (b,e), and June (c,f). Result from single model in top row and dual process model in bottom row.
...
top.row <- ggplot() +
  ...
  panel_border() +
  coord_sf(xlim = c(-74, -34.78), ylim = c(-33.8, 5.3))
```
