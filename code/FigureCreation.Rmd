...
This map is a 3 x 2 lattice, showing three characteristic annual time points in the year (represented as mean across all instances of that month) across the single and dual-process model. These values are re-scaled in order to be comparable across models.

Current rescaling (probability) by multiplying each predition by value $c$ such that:

$$ \sum{predictions} * c = \sum{positives} $$

This is done for each individual model (full, LRR, HRR), resulting in three different values of $c$.
...
Rescale raw outputs into probabilities and combine into one dataframe.

```{r}
pred.lowNHP <- pred.lowNHP %>%
  mutate(c = sum(case)/sum(prediction))%>%
  mutate(prob = prediction * c) %>%
...
pred.map <- na.omit(left_join(pred.all.2008, braz.map, by = "muni.no"))
...
```{r}
pdf(file = "figures/manuscript/Figure3Prob.pdf", width = 6.69, height = 4.75, family = "sans")

ggplot() +
  geom_sf(data = s.america, color = "gray80", fill = "gray80") +
  geom_sf(data = pred.map.subset, aes(fill = mean.prob), color = NA, size = 0) +
...
```{r}
pdf(file = "figures/manuscript/Figure3ProbQuant2008.pdf", width = 6.69, height = 4.75, family = "sans")

ggplot() +
  geom_sf(data = s.america, color = "gray80", fill = "gray80") +
  geom_sf(data = pred.map.subset, aes(fill = cut(prob, quantile(prob, probs = seq(0,1,0.2)))), color = NA, size = 0) +
...
single.plot <- ggplot() +
  geom_sf(data = s.america, color = "gray80", fill = "gray80") +
  geom_sf(data = pred.map.subset[pred.map.subset$model=="one.model",], aes(fill = cut(prob, quantile(prob, probs = seq(0,1,0.2)))), color = NA, size = 0) +
  scale_fill_brewer(palette = "OrRd", direction = 1, type = "seq",
                     guide = guide_legend(
                       title = "Ranked Quintiles",
                       direction = "horizontal",
                       ...
                       label.theme = element_text(size = 8, angle = 0)),
                    labels = c("0-20%", "21-40%", "41-60%", "61-80%", "81-100%")) +
  facet_grid( ~ monthLabel, dir = "h") +
  geom_sf(data = nhp.split.map, fill = NA, color = "gray50", size=0.1) +
  coord_sf(xlim = c(-74, -34.78), ylim = c(-33.8, 5.3), datum = NA) +
  my_theme_map()
...
double.plot <- ggplot() +
  geom_sf(data = s.america, color = "gray80", fill = "gray80") +
  geom_sf(data = pred.map.subset[pred.map.subset$model=="two.model",], aes(fill = cut(prob, quantile(prob, probs = seq(0,1,0.2)))), color = NA, size = 0) +
  scale_fill_viridis(option = "magma", direction = -1, discrete = T,
                     na.value = "gray70") +
  facet_grid(model ~ monthLabel, 
             labeller = labeller(
               model = c("one.model" = 'Single', "two.model" = 'Two Part')
