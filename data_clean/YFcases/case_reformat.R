...
for (j in 1:length(years)){
  yr <- years[j]
#read in data
  d <- read.csv(paste("YFcases/YF_20", yr,".txt", sep=""), sep=",", encoding="UTF-8",colClasses = c("character", rep("numeric",mon[j])))
  ...
  # 7. Save the work
    saveRDS(YF, file="YFcases/YFlong.rds")
    write.csv(YF, "YFcases/YFlong.csv")
